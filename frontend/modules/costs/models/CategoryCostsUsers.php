...
/**
 * This is the model class for table "category_costs_users".
 *
 * @property integer $id
 * @property integer $category_id
 * @property integer $user_id
 */
class CategoryCostsUsers extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    public static function categoriesOfUser($id)
    {
        if ($id && is_int($id)) {
            ...
            foreach(self::findAll(['user_id' => $id]) as $category) {
                $categoriesCosts = CategoriesCosts::findOne($category->category_id);
                $categoriesUser[$categoriesCosts->id] = $categoriesCosts ? $categoriesCosts->name : '';
            }

            $createdUserCategory = ArrayHelper::map(CategoriesCosts::find()
                                                    ->where(['user_id' => $id])
                                                    ->all(), 'id', 'name');
    ...
    public static function findCategoryInUser($userId)
    {
        $findStaticCategory = self::find()
                        ...
                        ->one();
        $findCreatedCategory = CategoriesCosts::find()
                                ->where(['user_id' => $userId])
                                ->one();
